...
class RoadSignRecognition:
    ...
    def classify(self, image):
        ...
        if self.bbox is not None:
            ...
            anchors1 = self.__class1 * scale
            anchors2 = self.__class2 * scale
            anchors1 = anchors1.astype(np.int32)
            anchors2 = anchors2.astype(np.int32)
            if image[anchors1[0][1], anchors1[0][0]] >= 150 and image[anchors1[0][3], anchors1[0][2]] >= 150:
                self.road_sign = "turn left"
            elif image[anchors2[0][1], anchors2[0][0]]< 250 and image[anchors2[0][3], anchors2[0][2]] < 250:
                self.road_sign = "turn right"
        else:
            self.road_sign = None
